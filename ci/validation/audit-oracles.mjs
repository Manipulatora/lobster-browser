 ...
 *
 * Exit code: non-zero if any oracle whose finding is marked FIXED fails â€” that is a regression.
 * Oracles for findings still OPEN are reported and counted, but do not fail the run: they are
 * tracked work, not new breakage. The report says exactly which is which, so "green" never means
...
function fmt(row) {
  ...
  return `  [${mark}] ${row.id.padEnd(38)} ${row.detail}`;
}
...
async function main() {
  ...
  if (!bin) {
    console.error(
      'No Lobium binary. These oracles measure the SHIPPING engine; running them against stock\n' +
        'Chromium would report a wall of failures that mean nothing. Set LOBSTER_LOBIUM_BIN.',
    );
    process.exitCode = 2;
    return;
  }
  ...
  const persona = deriveFingerprint(seed, { os: process.platform === 'win32' ? 'windows' : 'linux', engine: 'lobium' });
  ...
  const configPath = await writeLobiumConfig(configDir, buildLobiumConfig(persona, { seed }));

  // Serve a page from 127.0.0.1. `about:blank` is NOT a secure context, and `navigator.userAgentData`
  // â€” which several oracles read â€” is only exposed in one, so on about:blank they threw instead of
  // measuring anything. A loopback origin is "potentially trustworthy" per the spec, so it gives a
  // secure context with no certificate machinery.
  const server = createServer((_req, res) => {
    res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
    res.end('<!doctype html><meta charset="utf-8"><title>lobium oracles</title><body></body>');
  });
  ...
  const engine = await launchEngine({
    bin,
    headless: !headful,
    extraArgs: [lobiumConfigArg(configPath), ...gpuArgs],
  });
  try {
    await withCdpSession(engine.ws, async (session) => {
      ...
      for (const oracle of ORACLES) {
        const expression = typeof oracle.run === 'function' ? oracle.run(persona) : oracle.run;
        let pass = false;
        let inconclusive = false;
        let detail = '';
        try {
          const res = await cdpEvaluate(session, expression, { timeoutMs: 30_000 });
        ...
        rows.push({ ...oracle, run: undefined, pass, inconclusive, detail });
      }
    });
  } finally {
  ...
  const regressions = rows.filter((r) => !r.pass && !r.inconclusive && r.status === 'fixed');
  const stillOpen = rows.filter((r) => !r.pass && !r.inconclusive && r.status === 'open');
  const closedOpen = rows.filter((r) => r.pass && r.status === 'open');
  const unmeasured = rows.filter((r) => r.inconclusive);

  console.log(`\nseed ${seed} Â· ${headful ? 'headful' : 'HEADLESS (not release evidence)'}\n`);
  for (const r of rows) console.log(fmt(r));

  console.log(
    `\n${rows.filter((r) => r.pass).length}/${rows.length} oracles pass` +
  ...
  if (unmeasured.length) {
    console.log(
      `\nINCONCLUSIVE (${unmeasured.length}) â€” the probe could not run here, so this is neither\n` +
        ...
        unmeasured.map((r) => `${r.id}: ${r.detail.split('\n')[0]}`).join('\n  '),
    );
  }
    ...
    JSON.stringify(
      {
        ...
        stillOpen: stillOpen.map((r) => r.id),
        rows,
      },
      null,
      2,
