...
export class BillingPage {
  ...
  /**
   * The newest deposit still waiting on funds, with every field an instruction needs — or null.
   *
   * What the unfinished-payment banner and `showOpenDeposit` both read, derived from the deposit
   * list rather than stored so it cannot outlive the row: the moment a cancel or a confirmation
   * lands in a refresh, the banner goes with it. The field checks matter — a row missing its
   * address or amount predates those columns and cannot be rendered as a payable instruction.
   */
  protected readonly openDeposit = computed(
    () =>
      this.deposits().find(
        (d) =>
  ...
  /**
   * The smallest deposit the API accepts, in cents — the server's figure, not a local guess.
   *
   * The fallback only covers the gap before the overview lands (it mirrors the server's
   * MIN_DEPOSIT_CENTS); once loaded, the server's number wins, so the floor the field enforces is
   * the floor the API enforces.
   */
  protected readonly minDepositCents = computed(
    () => this.billing.overview()?.minDepositCents ?? 500,
  );
  ...
  /** The typed text as whole cents, or null while it is not yet a usable AND acceptable number. */
  protected readonly amountCents = computed<number | null>(() => {
    ...
    // FLOORED AT THE SERVER MINIMUM, so a below-minimum entry never becomes a payable amount.
    // A typed "0.001" used to round to 0 cents here, sail through as a number, and only fail at
    // the API as a bare 400 after Pay was pressed. Null keeps Pay disabled and puts the floor in
    // the hint under the field — the refusal happens while the user is still typing, with the
    // real figure attached.
    if (cents < this.minDepositCents()) return null;
    return cents;
  });
  ...
  private async init(): Promise<void> {
    ...
    // DELIBERATELY NO AUTOMATIC RESTORE of an open deposit here. This used to call
    // `restoreOpenDeposit()`, which put the newest still-pending address straight back on screen —
    // so one abandoned payment reappeared on every visit forever (nothing server-side ever closed
    // the row), looking like a payment the site generated by itself, "of 0", the moment the page
    // opened. Unnerving on exactly the page that handles money. The open deposit is still real
    // and still credits if paid, so it is offered instead: a banner names it and `showOpenDeposit`
    // brings the address back ON REQUEST — the memo-tag recovery the restore existed for survives,
    // as a choice rather than an ambush.
  ...
  /**
   * Put a still-open deposit back on screen — WHEN ASKED, from the unfinished-payment banner.
   *
   ...
   *
   * User-invoked, no longer called from `init()`: run on every load, this is what made one
   * abandoned payment reappear forever as an address nobody asked for (see the note in `init`).
   *
   * Only a deposit still waiting on funds can be shown ({@link openDeposit}); a confirmed or
   * expired one is history, and showing its address invites a second transfer to an address that
   * is no longer being watched.
   */
  protected showOpenDeposit(): void {
    if (this.instruction()) return;
    const open = this.openDeposit();
    if (!open) return;
  ...
  /**
   * Cancel: off the screen AND off the books.
   *
   * The local clear used to be the whole implementation, and it is why a cancelled payment kept
   * coming back — the row stayed `pending` on the server, and the next visit surfaced it again.
   * The screen is cleared FIRST, unconditionally: Cancel must always visibly work, and the server
   * call is fire-and-settle. If it fails, the worst case is the old behaviour (the banner names
   * the deposit again later, and the housekeeping sweep expires it within a week) — never an
   * error dialog in the way of someone leaving.
   */
  protected async dismissInstruction(): Promise<void> {
    ...
    if (!open) return;
    try {
      await this.billing.cancelDeposit(open.depositId);
    } catch {
      // Deliberately swallowed — see above. The server-side expiry sweep is the backstop.
    }
  }

  /** The banner's "Cancel it": same server call, without an instruction ever being on screen. */
  protected async cancelOpenDeposit(): Promise<void> {
    const open = this.openDeposit();
    if (!open) return;
    try {
      await this.billing.cancelDeposit(open.id);
    } catch {
      // The banner simply stays until the row really changes; the expiry sweep is the backstop.
    }
  }
