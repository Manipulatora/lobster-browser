...
| T-010 | Lobium: build environment + first Chromium build | F · Lobium | Claude | in-progress · real `lobium/build.sh` pipeline (fetch→sync→patch→gn→ninja, pinned ref) + `rebase.sh` + GN args + added-file strategy. **Compile is build-farm-gated** — cannot run in this sandbox (~150 GB + hours) |
| T-011 | Lobium: quilt series + first native patch + config channel POC | F · Lobium | Claude | in-progress · **config channel done + tested** (`lobium-config.ts` sidecar side, 4 tests) ⇄ native reader `lobium/src/lobium_fp_config.{h,cc}` + `patches/series`+`hooks.md` (build/config/navigator hooks authored). C++ finalized+compiled on the build machine |
