...
| T-011 | Lobium: quilt series + first native patch + config channel POC | F · Lobium | Claude | **done** · ✅ **native override BUILT + PROVEN**: [`hardware-concurrency-poc.patch`](../../lobium/patches/core/hardware-concurrency-poc.patch) → `navigator.hardwareConcurrency` 12→99→4 in C++ (no JS tell), ~2min incremental rebuild. Config channel: `lobium-config.ts` (tested) ⇄ native reader `lobium/src/lobium_fp_config.{h,cc}`. Deep-surface patches = next |
