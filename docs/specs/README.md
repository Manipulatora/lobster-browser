...
| [`fingerprint-parameters.md`](fingerprint-parameters.md) | The **~90-parameter** catalog across 18 surfaces (past the advertised 50+); the coherence engine as a 29-rule constraint set; the seed→native-config pipeline + determinism invariants; legacy CDP harness notes; the 11-panel editor UI grouping | B · Fingerprint (Claude) |
| [`fingerprint-catalog-provenance.md`](fingerprint-catalog-provenance.md) | Verified sources for fonts (MS Learn / Apple Support), Android models (Play CSV), and WebGL renderers (pci.ids / Apple product IDs); generator refresh procedure; acceptance floors | Fingerprint data |
